...
public class TenderCreation implements TenderCreationPort {
	...
	private static final QName SESSION_HEADER = new QName(NAMESPACE, "SessionHeader");
	...
	public ArrayOfCompanyReturn bidderSearch(String namePart, String sessionId) {
		if (!checkSessionId(sessionId)) {
			throw new HTTPException(SC_UNAUTHORIZED);
		}
		System.out.println("bidderSearch(namePart:" + namePart + ")");
		var result = new  ArrayOfCompanyReturn();
	...
	@Override
	public TenderCreationStatus checkTenderNumber(String nameAward, String namePrj, String sessionId) {
	...
	@Override
	public ArrayOfTenderConfigFormsets getFormsatzIds(int virtualVaId, boolean lots, String sessionId) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	public ArrayOfPublicProcurementLaw getPublicProcurementLaw(String sessionId) {
		...
		if (checkSessionId(sessionId)) {
			...
			result.add(new PublicProcurementLaw("Vergabe- und Vertragsordnung für Bauleistungen (VOB)", "VOB/EU", "390",
					true));
			result.add(new PublicProcurementLaw("ABCD - Vergabe- und Vertragsordnung. für Leistungen", "VOL/A", "47",
					false));
			result.add(new PublicProcurementLaw("Vergabe- und Vertragsordnung. für Leistungen (VOL)", "VOL/EU", "437",
					true));
			result.add(new PublicProcurementLaw("Vergabe- und Vertragsordnung. für frei. Leistungen", "LHO", "169",
					false));
			result.add(new PublicProcurementLaw("Vergabe- und Vertragsordnung. für frei. Leistungen (VgV)", "VgV",
					"484", true));
	...
	private String getSeesionId() {
		...
		Header header = headers.get(SESSION_HEADER, false);
		if (header == null)
			return "";
		...
		try {
			...
			while (reader.hasNext()) {
				if (reader.next() == XMLStreamConstants.START_ELEMENT && reader.getLocalName().equals("PHPSESSID")) {
	...
	@Override
	public ArrayOfSpecificsPublicProcurementLaw getSpecificPublicProcurementLaw(int vvId, String sessionId) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public String openingAction(int agLvId, Object token, Object addPaperBidders, Object finishOpening,
			Object vobagUser) {
		// TODO Auto-generated method stub
		return null;
	}
